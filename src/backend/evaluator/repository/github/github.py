...
class Github:
    ...
    def save(self, path):
        ...
        return {
            ...
            "organization_metadata": os.path.join(path, "organization", f'{self.metadata["organization"]["name"]}.json')
            if self.metadata["organization"]
            else None,
            "members_metadata": [
                os.path.join(path, "members", f"{member}.json") for member in self.metadata["members"]
            ],
        }
