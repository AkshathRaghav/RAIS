...
class Depot:
    ...
    def save(self, paper, github):
        ...
        if paper:
            ...
            mapping[paper.paper_url] = {**paper_paths}
        if github:
            repo_paths = github.save(self.repo_path)
            mapping[f"{github.owner}/{github.repo}"] = {**repo_paths}
    ...
    def load_git(self, url, branch):
        ...
        for run in mapping:  # each saved object, logic = (Paper | Github)
            if repo_name in run.keys():
                ...
                github_obj = Github(
                    owner=url.split("/")[-2], repo=url.split("/")[-1], branch=branch if branch else "main", depot=None
                )
            ...
            if paper in run.keys():
                ...
                for author in run[paper]["authors"]:
                    with open(author, "r") as file:
                        paper_obj.author_metadata[
                            author.replace("../depot/papers/authors/", "").replace(".json", "")
                        ] = json.load(file)
